        ...
        handleOutsideClick = (e) => {
          ...
          excludeElements.forEach((refName) => {
            if (!clickedOnExcludedElement) {
              ...
              if (
                typeof excludedElement !== "undefined" &&
                Array.isArray(excludedElement)
              ) {
                clickedOnExcludedElement =
                  excludedElement[0] && excludedElement[0].contains(e.target);
              } else if (
                typeof excludedElement !== "undefined" &&
                !Array.isArray(excludedElement)
              ) {
          ...
          const searchInputElement = e.target.classList.contains(
            vnode.context.searchInputClassName
          );
