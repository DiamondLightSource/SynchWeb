<template>
  ...
  <div class="content">
    ...
    <template slot="content" slot-scope="{ row }">
      ...
      <validation-provider tag="td" :rules="getValidationRules('thickness')" name="Thickness" :vid="'thickness-'+row['LOCATION']" v-slot="{ errors }">
        <base-input-text v-model="row['THICKNESS']" :quiet="true" :errorMessage="errors[0]"/>
      </validation-provider>
      <validation-provider tag="td" :rules="getValidationRules('background')" name="BackGround_1" :vid="'background_1-'+row['LOCATION']" v-slot="{ errors }">
        <base-input-text v-model="row['BACKGROUND1']" :quiet="true" :errorMessage="errors[0]"/>
      </validation-provider>
      <validation-provider tag="td" :rules="getValidationRules('background')" name="BackGround_2" :vid="'background_2-'+row['LOCATION']" v-slot="{ errors }">
        <base-input-text v-model="row['BACKGROUND2']" :quiet="true" :errorMessage="errors[0]"/>
      </validation-provider>
      <validation-provider tag="td" name="QRange" :vid="'qrange-'+row['LOCATION']" v-slot="{ errors }">
        <base-input-select v-model="row['QRANGE']" :options="qRangeOptions" optionTextKey="TEXT" optionValueKey="ID" :quiet="true" :errorMessage="errors[0]"/>
      </validation-provider>
    </template>
