  ...
  <div class="content">
    ...
    <table-component
      ...
      <template slot="content" slot-scope="{ row }" v-if="editRowLocation && editRowLocation == row['LOCATION']">
        <td>{{row['LOCATION']}}</td>
        <validation-provider tag="td" :name="'Acronym-'+sample['LOCATION']" :rules="sample['NAME'] ? 'required|min_value:1' : ''" v-slot="{ errors }"><sw-select-input v-model="sample['PROTEINID']" name="proteins" :options="availableProteins" optionValueKey="PROTEINID" optionTextKey="ACRONYM" :quiet="true" :errorMessage="errors[0]"/></validation-provider>
        ...
        <td><sw-select-input v-model="sample['COLUMN']" name="purification" :options="purificationColumns" optionValueKey="PURIFICATIONCOLUMNID" optionTextKey="NAME"/></td>
      ...
      <template slot="actions" slot-scope="{ row }">
        ...
        <span v-else>
          ...
          <a class="button" href="" @click.prevent="editRowLocation = ''"><i class="fa fa-times"></i></a>
        </span>
...
export default {
  ...
  methods: {
    onSaveSample: function(row) {
      ...
      this.inputValue[+row['LOCATION']-1] = Object.assign(this.inputValue[+row['LOCATION']-1], this.sample)
      this.editRowLocation = ''
      this.$emit('save-sample')
      // Reset temporary sample model
      this.sample = Object.assign(this.sample, {})
    },
    onEditSample: function(row) {
      console.log("Reset sample to edit = " + JSON.stringify(row))
