    ...
    function _pies()
    {
        ...
        foreach ($dc as &$d)
        {
            ...
            if (!array_key_exists('EDGE', $d))
                $d['EDGE'] = 0;
            if (!array_key_exists('AITIME', $d))
                $d['AITIME'] = 0;
            if (!array_key_exists('CENTTIME', $d))
                $d['CENTTIME'] = 0;
            if (!array_key_exists('FAULT', $d))
                $d['FAULT'] = 0;
            ...
            foreach (array('SUP', 'DCTIME', 'LEN', 'R', 'REM', 'T', 'EDGE') as $nf)
                $d[$nf] = number_format($d[$nf], 2, '.', '');
        ...
        if ($this->has_arg('visit'))
            $this->_output(sizeof($dc) ? $dc[0] : array());
        else
            $this->_output(array('VISITS' => $dc, 'AVERAGE' => $avgs));
    }
    ...
    function _overview()
    {
        ...
        if ($this->has_arg('ty'))
        {
            ...
            if ($this->arg('ty') == 'monthly')
                $where .= " AND TIMESTAMPDIFF('DAY', s.startdate, CURRENT_TIMESTAMP) <= 28";
            if ($this->arg('ty') == 'weekly')
        ...
        foreach ($dc as &$d)
        {
            ...
            foreach (array('USED', 'REM', 'LEN', 'DCH', 'MDCH', 'DC', 'SCH', 'MSCH', 'SC', 'MSLH', 'SLH', 'SL', 'MX') as $t)
                if ($d[$t])
