...
class Exp extends Page
{
    ...
    {
        $this->haltIfLackingPermission('manage_detector');

        if (!$this->has_arg('DETECTORTYPE'))
            $this->_error('No detector type specified');
        if (!$this->has_arg('DETECTORMANUFACTURER'))
            $this->_error('No detector manufacturer specified');
        if (!$this->has_arg('DETECTORMODEL'))
            $this->_error('No detector model specified');

        $args = array($this->arg('DETECTORTYPE'), $this->arg('DETECTORMANUFACTURER'), $this->arg('DETECTORMODEL'));
        foreach (array('DETECTORPIXELSIZEHORIZONTAL', 'DETECTORPIXELSIZEVERTICAL', 'DETECTORDISTANCEMIN', 'DETECTORDISTANCEMAX', 'DENSITY', 'COMPOSITION', 'DETECTORMINRESOLUTION', 'DETECTORMAXRESOLUTION', 'DETECTORROLLMIN', 'DETECTORROLLMAX', 'SENSORTHICKNESS') as $e)
            array_push($args, $this->has_arg($e) ? $this->arg($e) : null);
    ...
    function _update_detector()
    {
        $this->haltIfLackingPermission('manage_detector');
        if (!$this->has_arg('DETECTORID'))
            $this->_error('No detector specified');
    ...
    function _scan_services()
    {
        ...
        if ($this->has_arg('SCANPARAMETERSSERVICEID')) {
            if (sizeof($rows))
                ...
                $this->_error('No such scan service');
    ...
    {
        $this->haltIfLackingPermission('manage_service');

        if (!$this->has_arg('NAME'))
            $this->_error('No name specified');
        if (!$this->has_arg('DESCRIPTION'))
            $this->_error('No description model specified');
    ...
    function _update_scan_service()
    {
        $this->haltIfLackingPermission('manage_service');
        if (!$this->has_arg('SCANPARAMETERSSERVICEID'))
            $this->_error('No service specified');
    ...
    function _scan_models()
    {
        ...
        if ($this->has_arg('SCANPARAMETERSMODELID')) {
            if (sizeof($rows))
                ...
                $this->_error('No such scan parameters model');
    ...
    function _add_scan_model()
    {
        ...
        if (!$this->has_arg('DATACOLLECTIONPLANID'))
            $this->_error('No data collection plan specified');
        if (!$this->has_arg('SCANPARAMETERSSERVICEID'))
            $this->_error('No parameter model specified');
        ...
        if (!sizeof($chk))
            $this->_error('No such data collection plan');
    ...
    function _update_scan_model()
    {
        if (!$this->has_arg('prop'))
            $this->_error('No proposal specified');
        if (!$this->has_arg('SCANPARAMETERSMODELID'))
            $this->_error('No model specified');
    ...
    function _dp_has_detectors()
    {
        ...
        if ($this->has_arg('DATACOLLECTIONPLANHASDETECTORID')) {
            if (sizeof($rows))
                ...
                $this->_error('No such data collection plan detector');
    ...
    function _dp_add_detector()
    {
        ...
        foreach (array('EXPOSURETIME', 'DISTANCE', 'ROLL') as $f)
            array_push($args, $this->has_arg($f) ? $this->arg($f) : null);
    ...
    function _dp_update_detector()
    {
        if (!$this->has_arg('prop'))
            $this->_error('No proposal specified');
        if (!$this->has_arg('DATACOLLECTIONPLANHASDETECTORID'))
            $this->_error('No data collection plan detector specified');
    ...
    function _get_beamline_setups()
    {
        ...
        if ($this->has_arg('BEAMLINESETUPID')) {
            if (sizeof($rows))
                ...
                $this->_error('No such beamline setup');
    ...
    {
        $this->haltIfLackingPermission('manage_params');
        if (!$this->has_arg('BEAMLINENAME'))
    ...
    function _update_beamline_setup()
    {
        $this->haltIfLackingPermission('manage_params');
        if (!$this->has_arg('BEAMLINESETUPID'))
            $this->_error('No beamline setup specified');
    ...
    function _get_experiment_types()
    {
        ...
        if (!sizeof($rows))
            $this->_error('No experiment types found');
