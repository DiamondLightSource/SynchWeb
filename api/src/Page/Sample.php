...
class Sample extends Page
{
    ...
    function _add_simple_sample()
    {
        ...
        foreach ($this->args as $model => $attrs) {
            ...
            if (substr($model, 0, strpos($model, '_', 0)) != 'sample') {
                ...
                if (!$attrs->crystal)
                    $this->_error('No crystal defined');
                if (!$attrs->capillary)
                    $this->_error('No capillary defined');
                if (!$attrs->capillaryPhase)
                    $this->_error('No capillary material defined');
                if (!$attrs->phase)
                    $this->_error('No phase defined');
                if (!$attrs->container)
                    $this->_error('No container defined');
                if (!$attrs->DEWARID)
                    $this->_error('No default dewar defined');
                ...
                if (!array_key_exists('PROTEINID', $phase) && !array_key_exists('ACRONYM', $phase))
                    $this->_error('No protein id or acronym');
                if (!array_key_exists('ACRONYM', $capillaryPhase))
                    $this->_error('No protein acronym for capillary material');
                if (!array_key_exists('NAME', $crystal))
                    $this->_error('No crystal name specified');
                if (!array_key_exists('NAME', $capillary) || !array_key_exists('CRYSTALID', $capillary))
                    $this->_error('No capillary name specified');
                if (!array_key_exists('NAME', $container))
                    $this->_error('No container name specified');
                if (!array_key_exists('CONTAINERTYPE', $container))
    ...
    function _add_sample_component()
    {
        if (!$this->has_arg('prop'))
            $this->_error('No proposal specified');
        if (!$this->has_arg('ABUNDANCE'))
            $this->_error('No amount specified');
        if (!$this->has_arg('COMPONENTID'))
            $this->_error('No component specified');
        if (!$this->has_arg('BLSAMPLETYPEID'))
            $this->_error('No crystal specified');
        ...
        if (!sizeof($check))
            $this->_error('No such blsampletype');
    ...
    function _remove_sample_component()
    {
        ...
        if (!$this->has_arg('scid'))
            $this->_error('No crystal/component specified');
    ...
    {
        if (!$this->arg('ssid'))
            $this->_error('No sub sample specified');
    ...
    function _add_sub_sample()
    {
        ...
        if (!$this->has_arg('X'))
            $this->_error('No x position specified');
        if (!$this->has_arg('Y'))
            $this->_error('No y position specified');
    ...
    function _delete_sub_sample()
    {
        ...
        if (!$can_delete)
            $this->_error('Cannot delete that subsample as it is referenced by another entity: ' . $ref);
    ...
    function _update_sample_components($initial, $final, $amounts, $crystalid)
    {
        ...
        foreach ($rem as $r)
            $this->db->pq("DELETE FROM blsampletype_has_component WHERE blsampletypeid=:1 AND componentid=:2", array($crystalid, $r));
        foreach ($add as $a)
    ...
    {
        if (!$this->has_arg('pid'))
            $this->_error('No proteinid specified');
    ...
    function _update_sample()
    {
        ...
        $cfields = array('PROTEINID', 'SPACEGROUP', 'CELL_A', 'CELL_B', 'CELL_C', 'CELL_ALPHA', 'CELL_BETA', 'CELL_GAMMA', 'ABUNDANCE', 'THEORETICALDENSITY', 'COLOR');
    ...
    function _add_protein()
    {
        ...
        if (!$this->has_arg('prop'))
            $this->_error('No proposal specified');
        if (!$this->has_arg('ACRONYM'))
            $this->_error('No protein acronym');
        ...
        if ($valid_components) {
            ...
            if (!$this->staff) {
                ...
                if (sizeof($chkext))
                    $this->_error('No such protein to clone from');
            }
        }
    ...
    function _add_crystal()
    {
        ...
        if ($this->has_arg('COMPONENTIDS'))
            $this->_update_sample_components(array(), $this->arg('COMPONENTIDS'), $this->arg('COMPONENTAMOUNTS'), $crysid);
    ...
    function _update_sample_in_group()
    {
        ...
        if (!$this->has_arg('BLSAMPLEGROUPSAMPLEID'))
            $this->_error('No group sample id specified');
    ...
    function _get_spacegroups()
    {
        ...
        if (!sizeof($rows)) {
            if ($this->has_arg('SPACEGROUPID'))
                ...
                $this->_error('No spacegroup types found');
        }
