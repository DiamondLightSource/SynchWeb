...
class Cell extends Page
{
    ...
    function _cells($output = true)
    {
        ...
        foreach (array('a', 'b', 'c', 'al', 'be', 'ga') as $p) {
            if (!$this->has_arg($p))
                $this->_error('One or more unit cell parameters are missing');
    ...
    function _beamlines()
    {
        ...
        foreach ($facility_pdb_ident as $i)
    ...
    function _process_pdbs()
    {
        ...
        foreach ($rows as $r) {
            ...
            foreach ($facility_pdb_ident as $i)
                $bl_to_match = str_replace($i . ' ', '', $bl_to_match);
            if ($bl_to_match == strtoupper($r['BL']))
                $blmatch = 1;
    ...
    {
        if (!$this->has_arg('pdb'))
            $this->_error('No pdb file specified');
