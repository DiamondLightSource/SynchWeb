...
class Fault extends Page
{
    ...
    function _add_system()
    {
        ...
        if (!$this->has_arg('NAME'))
            $this->_error('Please specify a system name');
    ...
    function _add_component()
    {
        ...
        if (!$this->has_arg('NAME'))
            $this->_error('Please specify a component name');
        if (!$this->has_arg('SYSTEMID'))
            $this->_error('Please specify a system id');
    ...
    function _add_subcomponent()
    {
        ...
        if (!$this->has_arg('NAME'))
            $this->_error('Please specify a subcomponent name');
        if (!$this->has_arg('COMPONENTID'))
            $this->_error('Please specify a component id');
    ...
    function _update_system()
    {
        ...
        if (!$this->has_arg('SYSTEMID'))
            $this->_error('No system specified');
    ...
    function _update_subcomponent()
    {
        ...
        if (!$this->has_arg('SUBCOMPONENTID'))
            $this->_error('No subcomponent specified');
    ...
    function _edit($id, $typ)
    {
        ...
        if (!array_key_exists($typ, $types))
            $this->_error('That type doesnt exists');

        $ty = $types[$typ];

        $check = $this->db->pq('SELECT ' . $ty[0] . 'id as id FROM bf_' . $ty[0] . ' WHERE ' . $ty[0] . 'id=:1', array($id));
        if (!sizeof($check))
            $this->_error("That $typ doesnt exist");
        ...
        $bl_old = array();
        foreach ($bl_temp as $b)
            $bl_old[$b['BEAMLINENAME']] = $b['ID'];
