...
class DC extends Page
{
    ...
    function _data_collections($single = null)
    {
        ...
        $q = "
                ...
                SELECT
                    ...
                    1 as dcg,
                    'A' as runstatus,
                    es.beamsizehorizontal as bsx,
                    es.beamsizevertical as bsy,
                    1 as overlap,
                    1 as flux,
                    'edge' as type,
                    es.jpegchoochfilefullpath as imp,
                    1 as run,
                    es.scanfilefullpath as filetemplate,
                    es.energyscanid as id,
                    1 as ni,
                    es.element as dir,
                    es.peakfprime as resolution,
                    es.exposuretime as exposuretime,
                    es.axisposition as axisstart,
                    es.peakfdoubleprime as numimg,
                    es.starttime as st,
                    es.transmissionfactor as transmission,
                    es.inflectionfprime as axisrange,
                    es.inflectionfdoubleprime as wavelength,
                    1 as energy,
                    es.comments,
                    es.peakenergy as epk,
                    es.inflectionenergy as ein,
                    ...
                    1 as scanparams,
                    1 as totaldose,
                    1 as detectorpixelsizehorizontal,
                    1 as detectorpixelsizevertical,
                    '' as detectormanufacturer,
                    '' as detectormodel,
                    TIMESTAMPDIFF(MINUTE, es.starttime, CURRENT_TIMESTAMP) as age,
                    ses.beamlinename as bl,
                    es.blsubsampleid,
                    '' as detectornumberofpixelsx,
                    '' as detectornumberofpixelsy,
                    ses.archived,
                    ses.purgedProcessedData,
                    '' as rotationaxis,
                    '' as detector2theta
            ...
            SELECT
                ...
                1 as dcg,
                'A' as runstatus,
                xrf.beamsizehorizontal as bsx,
                xrf.beamsizevertical as bsy,
                1 as overlap,
                1 as flux,
                'mca' as type,
                'A' as imp,
            ...
            SELECT
                ...
                ROUND(TIMESTAMPDIFF('SECOND', CAST(r.starttimestamp AS DATE), CAST(r.endtimestamp AS DATE)), 1) as bsx,
                1 as bsy,
                1 as overlap,
                1 as flux,
                'load' as type,
                r.actiontype as imp,
                1 as run,
                smp.code as filetemplate,
                r.robotactionid as id,
                1 as ni,
                r.samplebarcode as dir,
                r.containerlocation as resolution,
                r.dewarlocation as exposuretime,
                1 as axisstart,
                1 as numimg,
                ...
                1 as wavelength,
                1 as energy,
                r.message as comments,
                1 as epk,
                1 as ein,
                1 as wpk,
                1 as win,
                r.xtalsnapshotbefore as x1,
                r.xtalsnapshotafter as x2,
                'A' as x3,
                'A' as x4,
        ...
        foreach ($dcs as $i => &$dc) {
            ...
            if ($dc['TYPE'] == 'data') {
                ...
                if (!$dc['DCT']) {
                    if ($dc['AXISRANGE'] == 0)
                        $dc['DCT'] = 'Still Image';
                    if ($dc['AXISRANGE'] == 0 && $dc['NI'] > 1)
                        $dc['DCT'] = 'Grid Scan';
                    if ($dc['OVERLAP'] != 0)
                        $dc['DCT'] = 'Screening';
                    if ($dc['AXISRANGE'] > 0 && $dc['OVERLAP'] == 0)
    ...
    {
        if (!$this->has_arg('id'))
            $this->_error('No datacollection id specified');
    ...
    {
        if (!$this->has_arg('t'))
            $this->_error('No data type specified');
        if (!$this->arg('id'))
    ...
    {
        if (!($this->has_arg('a') && $this->has_arg('b') && $this->has_arg('c') && $this->has_arg('al') && $this->has_arg('be') && $this->has_arg('ga') && $this->has_arg('sg')))
            $this->_error('Missing parameters');
    ...
    {
        if (!$this->has_arg('DATACOLLECTIONID'))
            $this->_error('No datacollection specified');
