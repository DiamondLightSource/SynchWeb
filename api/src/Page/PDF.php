...
class PDF extends Page
{
    ...
    {
        if (!$this->user->hasPermission('view_manifest'))
    ...
    function _shipment_label()
    {
        ...
        $this->dewars = $this->db->pq("SELECT bl.beamlinename, bl.beamlineoperator, TO_CHAR(bl.startdate, 'DD-MM-YYYY') as st, d.transportvalue, d.customsvalue, d.code, d.barcode, count(cq.containerqueueid) as auto, count(c.containerid) as containers, GROUP_CONCAT(COALESCE(cr.barCode, c.code) LIMIT 10) as containersBarCode
                ...
                LEFT OUTER JOIN containerregistry cr ON cr.containerregistryid = c.containerregistryid
    ...
    function _visit_report()
    {
        ...
        foreach ($aps as &$a)
        {
            ...
            if (!array_key_exists($a['ID'], $ap))
                $ap[$a['ID']] = array();
            if (!array_key_exists($a['TYPE'], $ap[$a['ID']]))
                $ap[$a['ID']][$a['TYPE']] = array();
    ...
    {
        if (!($this->has_arg('cid') || $this->has_arg('did') || $this->has_arg('sid')))
            $this->_error('No container, dewar, or shipment', 'No container, dewar, or shipment was specified');
        if (!$this->has_arg('prop'))
        ...
        foreach ($containers as &$c)
        {
            ...
            $used = array();
            foreach ($c['SAMPLES'] as $r)
                array_push($used, $r['LOCATION']);
            $tot = array();
    ...
    function _generate_sheets()
    {
        ...
        if ($this->has_arg('labels'))
        {
            if (is_array($this->arg('labels')))
                ...
                $this->labels = array($this->arg('labels'));
        }
