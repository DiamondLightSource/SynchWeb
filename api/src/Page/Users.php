...
class Users extends Page
{
    ...
    function _add_group_permission()
    {
        ...
        if (sizeof($chk))
            $this->_error('That group already has the supplied permission');
    ...
    function _remove_group_permission()
    {
        ...
        $chk = $this->db->pq("SELECT usergroupid FROM usergroup_has_permission WHERE usergroupid=:1 and permissionid=:2", array($this->arg('gid'), $this->arg('pid')));
        if (!sizeof($chk))
            $this->_error('That group does not have the supplied permission');
    ...
    function _get_users()
    {
        ...
        foreach ($rows as &$r) {
            if ($r['PERSONID'] == $this->user->personid)
                $r['FULLNAME'] .= ' [You]';
        }
    ...
    function _add_user()
    {
        ...
        if (!$this->has_arg('LOGIN'))
            $this->_error('No login specified');
        if (!$this->has_arg('GIVENNAME'))
            $this->_error('No given name specified');
        if (!$this->has_arg('FAMILYNAME'))
            $this->_error('No family name specified');
        ...
        foreach ($lfields as $i => $f) {
            if ($this->has_arg($f)) {
                ...
                if (!$person['LABORATORYID']) {
                    ...
                    $this->db->pq('UPDATE laboratory SET ' . $c . '=:1 WHERE laboratoryid=:2', array($this->arg($f), $person['LABORATORYID']));
                $this->_output(array($f => $this->arg($f)));
            }
        }
    }
    ...
    function _add_group_user()
    {
        ...
        if (sizeof($chk))
            $this->_error('That group already contains the supplied person');
