...
class Imaging extends Page
{
    ...
    {
        $this->haltIfLackingPermission('schedules');
    ...
    function _update_schedule()
    {
        ...
        if (!$this->has_arg('shid'))
            $this->_error('No schedule specified');
    ...
    {
        $this->haltIfLackingPermission('schedule_comps');

        if (!$this->has_arg('SCHEDULEID'))
            $this->_error('No schedule specified');
        if (!$this->has_arg('OFFSET_HOURS'))
            $this->_error('No time offset specified');
        if (!$this->has_arg('INSPECTIONTYPEID'))
            $this->_error('No inspection type specified');
    ...
    function _update_schedule_component()
    {
        ...
        if (!$this->has_arg('shcid'))
            $this->_error('No schedule component specified');
        if (!$this->has_arg('OFFSET_HOURS'))
            $this->_error('No offset specified');
        if (!$this->has_arg('INSPECTIONTYPEID'))
    ...
    function _get_inspections()
    {
        ...
        if ($this->has_arg('ty')) {
            ...
            if ($this->arg('ty') == 'INCOMPLETE')
                $where .= " AND i.state != 'Completed'";
            if ($this->arg('ty') == 'SCHEDULED')
                $where .= " AND i.scheduledtimestamp IS NOT NULL AND i.state != 'Completed'";
            if ($this->arg('ty') == 'ADHOC')
                $where .= " AND i.schedulecomponentid IS NULL";
            if ($this->arg('ty') == 'MANUAL')
                $where .= " AND i.manual=1";
        }
    ...
    function _add_inspection()
    {
        ...
        if (!$this->has_arg('IMAGERID') && !$this->has_arg('TEMPERATURE'))
            $this->_error('No temperature specified');
    ...
    function _add_inspection_image()
    {
        ...
        if (!$this->has_arg('CONTAINERINSPECTIONID'))
            $this->_error('No inspection specified');
    ...
    {
        if (!$this->has_arg('imid'))
            $this->_error('No inspection image specified');
    ...
    function _get_auto_scores()
    {
        ...
        $inspection = array();
        foreach ($rows as $r) {
            if (!array_key_exists($r['BLSAMPLEIMAGEID'], $inspection))
                $inspection[$r['BLSAMPLEIMAGEID']] = array(
    ...
    {
        if (!$this->has_arg('imid'))
            $this->_error('No image specified');
    ...
    {
        if (!$this->has_arg('scid'))
            $this->_error('No screen specified');
    ...
    function _add_screen_componentgroups()
    {
        if (!$this->has_arg('collection'))
            $this->_error('No groups specified');

        foreach ($this->arg('collection') as $g) {
            if (array_key_exists('SCREENID', $g))
                $this->args['SCREENID'] = $g['SCREENID'];
            else
                unset($this->args['SCREENID']);

            if (array_key_exists('SCREENCOMPONENTGROUPID', $g))
                continue;

            if (array_key_exists('POSITION', $g))
                $this->args['POSITION'] = $g['POSITION'];
            else
    ...
    {
        if (!$this->has_arg('SCREENID'))
            $this->_error('No screen specified');
        if (!$this->has_arg('POSITION'))
            $this->_error('No position specified');
    ...
    {
        if (!$this->has_arg('collection'))
            $this->_error('No collection specified');

        foreach ($this->arg('collection') as $c) {
            if (array_key_exists('SCREENCOMPONENTID', $c))
                continue;

            $this->args['SCREENCOMPONENTGROUPID'] = $c['SCREENCOMPONENTGROUPID'];
            $this->args['COMPONENTID'] = $c['COMPONENTID'];

            if (array_key_exists('CONCENTRATION', $c))
                $this->args['CONCENTRATION'] = $c['CONCENTRATION'];
            else
                unset($this->args['CONCENTRATION']);

            if (array_key_exists('PH', $c))
                $this->args['PH'] = $c['PH'];
            else
                unset($this->args['PH']);
    ...
    {
        if (!$this->has_arg('SCREENCOMPONENTGROUPID'))
            $this->_error('No screen component group specified');
