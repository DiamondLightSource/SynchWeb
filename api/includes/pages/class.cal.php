<?php
    ...
    class Cal extends Page {
        ...
        function _export_ics() {
            ...
            // $user_tmp = $this->db->pq("SELECT u.name,u.fullname,s.sessionid 
            //     FROM blsession s 
            ...
            //     WHERE (s.beamlinename LIKE 'i02' OR s.beamlinename LIKE 'i03' OR s.beamlinename LIKE 'i04' OR s.beamlinename LIKE 'i04-1' OR s.beamlinename LIKE 'i24') AND s.startdate > TO_DATE(:1,'YYYY') AND iu.role='NORMAL_USER' $where", $args);
            
            $bls = implode("', '", $mx_beamlines);
            $user_tmp = $this->db->pq("SELECT pe.login, pe.givenname || '-' || pe.familyname as fullname
                FROM person pe
            ...
            foreach ($visits as $v) {
                ...
                if (!in_array($arg, $mx_beamlines)) $title = $v['BL'].': '.$title;
